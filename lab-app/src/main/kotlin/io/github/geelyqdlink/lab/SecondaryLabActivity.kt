...
class SecondaryLabActivity : Activity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        LabLog.event("SECONDARY_ACTIVITY_CREATED", "display=${display?.displayId}")
    ...
    private inner class PatternView : View(this) {
        ...
        override fun onDraw(canvas: Canvas) {
            ...
            canvas.drawText("VirtualDisplay / no MediaProjection", width * 0.08f, height * 0.48f, paint)
        ...
        override fun onTouchEvent(event: MotionEvent): Boolean {
            ...
            LabLog.event(
                "SECONDARY_TOUCH",
                "display=${display?.displayId} action=${event.actionMasked} x=${event.x.toInt()} y=${event.y.toInt()}",
            )
            invalidate()
            return true
        }
